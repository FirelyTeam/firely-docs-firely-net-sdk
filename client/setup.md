...
servers](http://wiki.hl7.org/index.php?title=Publicly_Available_FHIR_Servers_for_testing) you can use.
...
To specify the preferred format of the content to be used when communicating
...
described in the [Managing Return Content](http://www.hl7.org/fhir/http.html#2.21.0.5.2) section
of the specification. The default setting is to ask for the complete representation. If you want to
change that request, you can set the `ReturnPreference` attribute:
...
As described in the section on [serialization](parsing), there are two parser families: the legacy XML and Json parsers that have been in the SDK for years, and the improved ones already introduced in SDK 4. Each of these parsers can be configured
