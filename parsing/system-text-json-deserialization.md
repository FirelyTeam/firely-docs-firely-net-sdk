...
The error codes are defined as constants on the `FhirJsonException` class. Rather than maintaining a (probably outdated) table of codes here, it is preferable to glance the current set [from the code](https://github.com/FirelyTeam/firely-net-sdk/blob/develop/src/Hl7.Fhir.Base/Serialization/FhirJsonException.cs).
