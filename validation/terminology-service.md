...
```{note}
The `LocalTerminologyService` doesn't support all terminology features. To fully support all features a lot of terminology expertise is needed. So to make use of all advanced terminology features, use the `ExternalTerminologyServer`
...
```{note}
You can use a '*' to specify wildcards in the routing mechanism.
```
